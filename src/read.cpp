...
void ReadParameters(const char* fileinput) {
	...
	ArcProp.push_back("InvStep"); ArcDefault.push_back("y");
	...
	// Sustainability
	for (int j = 0; j < SustObj.size(); ++j) { SustMet.push_back( SustObj[j] ); }
...
vector< vector<double> > ReadBendersStepLength(const char* fileinput) {
    ...
    if ( file != NULL ) {
        for (;;) {
            ...
            // Remove comments and end of line characters
            CleanLine(line);
			vector<double> oneline(0);
			t_read = strtok(line,",");
			oneline.push_back( atof(t_read) );
