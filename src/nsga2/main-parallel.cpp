...
int main (int argc, char **argv) {
	...
	// Read indices
	ImportIndices();																	// JINXU: I think this can be erased
	...
	double events[(SLength[0] + IdxCap.GetSize()) * (Nevents+1)];						// JINXU: Remove this line
	ReadEvents(events, "prepdata/bend_events.csv");										// JINXU: Remove this line
	
	cout << "- Initialization done, now performing first generation" << endl;
	
	// -- Start evaluating 1A -- // 
	nsga2a->decodePop(nsga2a->parent_pop);
	nsga2a->evaluatePop(nsga2a->parent_pop, netplan, events);							// JINXU: Remove 'netplan' and 'events'
	...
	nsga2b->evaluatePop(nsga2b->parent_pop, netplan, events);							// JINXU: Remove 'netplan' and 'events'
	...
	for ( int i = 2; i <= nsga2a->ngen; i++ ) {
		...
		nsga2a->evaluatePop(nsga2a->child_pop, netplan, events);						// JINXU: Remove 'netplan' and 'events'
		...
		nsga2b->evaluatePop(nsga2b->child_pop, netplan, events);						// JINXU: Remove 'netplan' and 'events'
