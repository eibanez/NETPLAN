...
CNSGA2::CNSGA2(bool first) {
	randgen	= new CRand(RAND_SEED);
	if (first)  fileio = new CFileIO(this);
...
void CNSGA2::InitPop (population *pop) {
	for ( int i = 0; i < popsize; i++ )
		InitInd(&(pop->ind[i]));
}

// Randomly initialize individuals
void CNSGA2::InitInd(individual *ind) {
...
void CNSGA2::evaluatePop (population *pop) {
	for (int i=0; i<popsize; i++)
		evaluateInd (&(pop->ind[i]));
	return;
}

/* Routine to evaluate objective function values and constraints for an individual */
void CNSGA2::evaluateInd (individual *ind) {
