...
string Arc::CapArcColumns(bool isBenders) const {
    string temp_output = "";
	
	// If investment is allowed,
    if ( Get("OpMax") != "Inf" && Get("TransInfr") == "" ) {
		if ( !isBenders ) {
			// Add capacity as an upper bound for flow
			temp_output += "    cap" + Get("Code") + " ub" + Get("Code") + " -" + Get("StepLength") + "\n";
		}
	...
	if ( Get("CapacityFactor") != "0" ) {
		temp_output += "    cap" + Get("Code") + " pk" + Get("To") + Get("ToStep") + " " + Get("CapacityFactor") + "\n";
	}
	
    return temp_output;
}

string Arc::StepLengthBenders() const {
    string temp_output = "";
	
	// If investment is allowed,
    if ( Get("OpMax") != "Inf" && Get("TransInfr") == "" /*&& Time() == 1*/ ) {
		// Base case
		temp_output += Get("StepLength");
		for (int event = 1; event <= Nevents; ++event) {
			// For events
			string property = "CapacityLoss" + ToString<int>(event);
			temp_output += "," + ToString<double>( GetDouble("StepLength") * GetDouble(property) );
		}
		temp_output += "\n";
	}
    return temp_output;
