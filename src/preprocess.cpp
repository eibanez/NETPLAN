...
int main() {
	...
	string Ychar = SName.substr(0,1);
	
	// *** Start writing one single MPS file (no Benders) ***
	afile.open("netscore.mps");
	afile << "NAME\n";
	afile << "ROWS\n";
	...
	for (int j = 0; j < SustMet.size(); ++j)
		for (int i = 1; i <= nyears; ++i)
			afile << "    " << SustMet[j] << "_" << Ychar << i << " " << SustMet[j] << Ychar << i << " -1\n";
	
	// Reserve margin
	for (unsigned int i = 0; i < Nodes.size(); ++i)
		afile << Nodes[i].NodeRMColumns();
	
	for (unsigned int i = 0; i < Arcs.size(); ++i)
		afile << Arcs[i].InvArcColumns();
	
	for (unsigned int i = 0; i < Arcs.size(); ++i)
		afile << Arcs[i].CapArcColumns( false );
	
	for (unsigned int i = 0; i < Arcs.size(); ++i)
		afile << Arcs[i].ArcColumns();
	
	for (unsigned int i = 0; i < Nodes.size(); ++i)
		afile << Nodes[i].NodeColumns();
	...
	// *** Create multiple MPS files (one per year, for Benders decomp.) ***
	...
	for (int j = 0; j < SustMet.size(); ++j)
		for (int i = 1; i <= nyears; ++i)
			myfile[i] << "    " << SustMet[j] << "_" << Ychar << i << " " << SustMet[j] << Ychar << i << " -1\n";
	...
	//vector<string> DcOutput = DCFlowColumns(ListDCNodes, ListDCArcs);   // Defined above
	for (unsigned int i = 1; i < DcOutput.size(); ++i)
		myfile[i] << DcOutput[i];
	...
	afile.open("prepdata/bend_steplengths.csv");
	for (unsigned int i = 0; i < Arcs.size(); ++i)
